...
<script type="text/javascript">	
	...
	function insertForm(){
		location.href="uqainsertform.do";
	}
...
<!-- 		<th>delflag</th> -->
	...
	<c:choose>
		...
		<c:otherwise>
			<c:forEach items="${list}" var="dto">
				<tr>
					<td><input type="hidden" name="chk" value="${dto.seq}"/></td>
					<td>${count=count+1}</td>
					<td>${dto.id}</td>
					<c:choose>
						<c:when test="${dto.delflag eq 'N'}">
							<td style="width:300px;">
								...
								<a href="udetailboard.do?seq=${dto.seq}">${dto.title}</a>
							</td>					
						</c:when>
						<c:otherwise>
	...
	<tr>
		<td colspan="6">
			<input class="btn btn-primary" type="button" value="글추가" onclick="insertForm()" />
			<input class="btn btn-primary" type="button" value="메인" onclick="userMain()" />
